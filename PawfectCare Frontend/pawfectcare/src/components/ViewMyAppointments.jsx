...
const ViewMyAppointments = () => {
  ...
  const fetchAppointments = async (customerId) => {
    try {
      const response = await axios.get(`http://localhost:5021/api/Appointment/customer/${customerId}`);
  ...
  const cancelAppointment = async (appointmentId) => {
    try {
      await axios.delete(`http://localhost:5021/api/Appointment/cancel/${appointmentId}`);
