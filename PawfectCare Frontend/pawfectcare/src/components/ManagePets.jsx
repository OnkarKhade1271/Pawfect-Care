...
const ManagePets = () => {
  ...
  const fetchPets = async (customerId) => {
    const response = await fetch(`http://localhost:5021/api/Pet/pets/${customerId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    const response = editing
      ? await fetch(`http://localhost:5021/api/Pet/${editPetId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ ...pet, customerId }),
        })
      : await fetch(`http://localhost:5021/api/Pet/CreatePet`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ ...pet, customerId }),
        });
  ...
  const handleDelete = async (petId) => {
    const response = await fetch(`http://localhost:5021/api/Pet/${petId}`, {
      method: "DELETE",
    });
