...
const BookAppointment1 = () => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      const response = await fetch("http://localhost:5021/api/Services");
  ...
  const checkSlotAvailability = async () => {
    try {
      const response = await fetch(
        `http://localhost:5021/api/Appointment/viewslots?appointmentDate=${formatDate(appointmentDate)}&appointmentTime=${selectedTimeSlot}`
      );
      const data = await response.json();
